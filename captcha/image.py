...
class ImageCaptcha:
    ...
    def create_captcha_image(
        ...
        for c in chars:
            if random.random() > self.word_space_probability:
                ...
                char_list_dummy.append("")
                char_index += 1
            images.append(self._draw_character(c, draw, color))
            char_list_dummy.append(c)
            char_image_indexes.append(char_index)
            char_index += 1
        ...
        for i, im in enumerate(images):
            ...
            # by yty, test bounding box
            if i in char_image_indexes:
                x1,y1,x2,y2= offset, int((self._height - h) / 2), offset+w, int((self._height - h) / 2)+h
                # draw2 = Draw(image)
                #draw2.rectangle((x1,y1,x2,y2),outline="red")
                with open(anno_file_path, "a") as file:
                    # Write each set of values to a line in the text file
                    file.write(f"{cat2class[char_list_dummy[i]]} {(x1+x2)*0.5/width} {(y1+y2)*0.5/height} {float(w)/width} {float(h)/height}\n")
    ...
    def generate_image(self, image_path:str, cat2class:dict,chars: str) -> Image:
        """Generate the image of the given characters.

        :param chars: text to be generated.
        """
        image_name = image_path[:image_path.rfind(".")]
        anno_file_path = image_name+".txt"
        background = random_color(238, 255)
        color = random_color(10, 200, random.randint(220, 255))
        im = self.create_captcha_image(anno_file_path,cat2class,chars, color, background)
